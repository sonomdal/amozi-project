...
| Actor | Description |
|:------:|:----------------------------------------------|
| User | 사용자의 개인 정보를 입력하고 수정할 수 있다. |

3) Check Weather Information
   
| Actor | Description |
|:------:|:----------------------------------------------|
| User | 사용자가 날짜와 위치를 선택한다. |
| System | 날씨 API에 날짜와 위치 정보를 전달하여 날씨 데이터를 수신한다. |
...
5) Select Body Condition

| Actor | Description |
|:------:|:----------------------------------------------|
| User | 현재 몸 상태(감기, 피곤함, 추위를 많이 탐 등)을 선택하여 코디 추천에 반영할 수 있다. |

6) Request Outfit Recommendation
   
| Actor | Description |
|:------:|:----------------------------------------------|
| User | 날씨와 개인 정보를 기반으로 의상 추천을 요청할 수 있다. |
| System | 날씨, 상황, 몸 상태, 옷장 아이템 정보를 기반으로 추천 결과를 생성한다. |
...
| Purpose | Approach | Dynamics | Goals |
|:--------|:---------|:---------|:------|
| 사용자가 처한 상황을 추천에 반영한다. | 상황 선택 기능을 제공한다. | 사용자가 상황을 선택하면 해당 정보가 코디 추천 알고리즘에 반영된다. | 상황에 적합한 코디 추천을 가능하게 한다. |

5) Select Body Condition

| Purpose | Approach | Dynamics | Goals |
|:--------|:---------|:---------|:------|
| 사용자의 몸 상태를 추천에 반영한다 | 몸 상태 선택 기능을 제공한다. | 사용자가 하나 이상의 몸 상태를 선택하면 해당 정보가 추천 알고리즘에 반영된다. | 사용자 몸 상태에 적합한 개인화된 코디 추천을 제공한다. |

5) Request Outfit Recommendation
   
| Purpose | Approach | Dynamics | Goals |
|:--------|:---------|:---------|:------|
| 사용자에게 적절한 코디를 추천한다. | AI API를 활용한 추천 시스템을 구현한다. | 날씨, 상황, 몸 상태, 옷장 아이템 정보를 AI에 전달하여 개인화된 추천 결과를 생성한다. | 사용자 만족도를 높이고 의사결정 시간을 줄이며 개인화된 추천 정확도를 향상시킨다. |
...
| Purpose | Approach | Dynamics | Goals |
|:--------|:---------|:---------|:------|
| 추천 결과에 대한 사용자 반응을 수집한다. | 코디 추천 결과에 대한 피드백 입력 기능을 제공한다. | 사용자가 추천 결과에 대한 평가를 별점을 통해 입력하면 시스템이 이를 저장하고 이후 추천에 반영한다. | 추천 시스템의 성능을 지속적으로 개선한다. |
